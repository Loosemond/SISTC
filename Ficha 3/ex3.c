...
int main() 
{
  ...
  sigfillset(&new_mask);// cria um conjunto cheio - vai estar tudo bloqueado
  sigdelset(&new_mask, SIGTERM); // retiramos os sinais que nao queremos bloquear
  ...
  while(1){
    ...
    print_pending();
    r = sleep(10); // se o sleep for interrompido ele retorna o tempo que falta para a variavel r
    if(r>0)
